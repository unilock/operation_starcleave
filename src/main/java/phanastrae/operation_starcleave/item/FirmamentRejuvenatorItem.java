...
public class FirmamentRejuvenatorItem extends Item implements ProjectileItem {
    private static final ProjectileItem.Settings PROJECTILE_SETTINGS = ProjectileItem.Settings.builder().uncertainty(ProjectileItem.Settings.DEFAULT.uncertainty() * 0.1F).power(ProjectileItem.Settings.DEFAULT.power() * 1.5F).build();
    ...
    public ProjectileEntity createEntity(World world, Position pos, ItemStack stack, Direction direction) {
        FirmamentRejuvenatorEntity firmamentRejuvenatorEntity = new FirmamentRejuvenatorEntity(world, pos.getX(), pos.getY(), pos.getZ());
        firmamentRejuvenatorEntity.setItem(stack);
        return firmamentRejuvenatorEntity;
    }
}
